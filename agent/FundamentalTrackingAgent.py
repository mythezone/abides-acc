...
class FundamentalTrackingAgent(TradingAgent):
    ...
    def measureFundamental(self):
        ...
        obs_t = self.oracle.observePrice(
            self.symbol, self.agent_current_time, sigma_n=0
        )
        self.fundamental_series.append(
            {"FundamentalTime": self.agent_current_time, "FundamentalValue": obs_t}
        )
    ...
    def writeFundamental(self):
        ...
        self.writeLog(
            dfFund,
            filename="fundamental_{symbol}_freq_{self.log_frequency}_ns".format(
                self.symbol
            ),
        )
