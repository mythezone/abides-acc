...
class EtfMarketMakerAgent(EtfArbAgent):
    ...
    def __init__(
        ...
        self.inPrime = (
            True  # Determines if the agent also participates in the Primary ETF market
        )
    ...
    def wakeup(self, currentTime):
        ...
        if not self.prime_open:
            ...
            self.sendMessage(
                self.primeID, Message({"msg": "WHEN_PRIME_OPEN", "sender": self.id})
            )
            self.sendMessage(
                self.primeID, Message({"msg": "WHEN_PRIME_CLOSE", "sender": self.id})
            )
        ...
        # after market close.
            ...
            if self.switched_mkt and self.agent_current_time >= self.prime_open:
                self.getEtfNav()
    ...
    def receiveMessage(self, currentTime, msg):
        ...
        if msg.body["msg"] == "WHEN_PRIME_OPEN":
            ...
            log_print(
                "Recorded primary close: {}", self.kernel.fmtTime(self.prime_close)
            )
    ...
    def placeBasketOrder(self, quantity, is_create_order):
        order = BasketOrder(
            self.id, self.agent_current_time, "ETF", quantity, is_create_order
        )
