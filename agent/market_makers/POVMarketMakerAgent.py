...
class POVMarketMakerAgent(TradingAgent):
    ...
    def receiveMessage(self, currentTime, msg):
        ...
        if (
            msg.body["msg"] == "QUERY_TRANSACTED_VOLUME"
    ...
    def placeOrders(self, mid):
        ...
        for bid_price in bid_orders:
            log_print(
                f"{self.name}: Placing BUY limit order of size {self.order_size} @ price {bid_price}"
            )
            self.placeLimitOrder(self.symbol, self.order_size, True, bid_price)

        for ask_price in ask_orders:
            log_print(
                f"{self.name}: Placing SELL limit order of size {self.order_size} @ price {ask_price}"
            )
