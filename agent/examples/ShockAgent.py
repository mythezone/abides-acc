...
class ImpactAgent(TradingAgent):
    ...
    def __init__(
        ...
        # Base class init.
        super().__init__(
            id, name, type, starting_cash=starting_cash, random_state=random_state
        )
        ...
        self.impact_duration = (
            impact_duration  # How long the agent should wait to submit the next trade
        )
    ...
    def wakeup(self, currentTime):
        ...
        if self.traded or (
            self.mkt_closed and (not self.symbol in self.daily_close_price)
        ):
            self.getLastTrade()
    ...
    def placeLimitOrder(self, symbol, quantity, is_buy_order, limit_price):
        super().placeLimitOrder(
            symbol, quantity, is_buy_order, limit_price, ignore_risk=True
        )
    ...
    def computeRequiredPrice(self, direction, shares):
        book = (
            self.known_asks[self.symbol] if direction else self.known_bids[self.symbol]
        )
