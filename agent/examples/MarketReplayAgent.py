...
class MarketReplayAgent(TradingAgent):
    ...
    def wakeup(self, currentTime):
        ...
        try:
            ...
            self.placeOrder(
                currentTime, self.historical_orders.orders_dict[currentTime]
            )
        except IndexError:
            log_print(
    ...
    def processOrders(self):
        ...
        if os.path.isfile(processed_orders_file):
            ...
            orders_df["BUY_SELL_FLAG"] = (
                orders_df["BUY_SELL_FLAG"]
                .astype(int)
                .replace(L3OrdersProcessor.DIRECTION)
            )
            orders_df["TIMESTAMP"] = (
                orders_df["TIMESTAMP"].astype(str).apply(convertDate)
            )
