    ...
    def add_agent(self, agent_type, symbols, num_agents=1):
        if agent_type == "ExchangeAgent":
            ...
            for i in range(num_agents):
                self.agents.append(
                    NoiseAgent(id=self.agent_count+i,
                               name=f"NoiseAgent {i}",
                               type="NoiseAgent",
                               symbol=symbols,
                               starting_cash=self.simulator.starting_cash,
                               wakeup_time=util.get_wake_time(self.simulator.mkt_open,self.simulator.mkt_close),
                               log_orders=self.simulator.exchange_log_orders,
                               random_state=np.random.RandomState(seed=np.random.randint(low=0, high=2 ** 32, dtype='uint64')))
                )
