...
def load_series(log_dir: str, symbol: str, kind: str = "ohlc"):
    f = os.path.join(log_dir, symbol, f"{kind}.csv")
...
def compare_symbol(src_dir: str, cal_dir: str, symbol: str, out_dir: str, kind: str = "ohlc"):
    a = load_series(src_dir, symbol, kind)
    b = load_series(cal_dir, symbol, kind)
    if a is None or b is None:
        print(f"[skip] missing data for {symbol} kind={kind}")
        return
    # align on time
    ...
    if kind == "ohlc" and key in a.columns and key in b.columns:
        ...
        plt.legend(); plt.title(f"{symbol} {kind} compare (MAE={mae:.4f})")
        plt.tight_layout()
        outp = os.path.join(out_dir, f"{symbol}_{kind}.png")
        plt.savefig(outp)
        plt.close()
        print(f"[ok] {symbol} {kind} MAE={mae:.4f} -> {outp}")
    else:
        print(f"[skip] unsupported kind or columns for {symbol}")
...
def main():
    ...
    if not symbols:
        ...
        symbols = [d for d in os.listdir(args.src) if os.path.isdir(os.path.join(args.src, d))]

    for s in symbols:
        compare_symbol(args.src, args.cal, s, args.out, kind="ohlc")
