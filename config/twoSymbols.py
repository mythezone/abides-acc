...
for i, x in enumerate(mm):
    ...
    agent_types.extend(
        ["EtfMarketMakerAgent {}".format(strat_name) for j in range(x[0])]
    )
    agent_count += x[0]
...
for i, t1 in zip(range(latency.shape[0]), agent_types):
    for j, t2 in zip(range(latency.shape[1]), agent_types):
        ...
        if j > i:
            ...
            if ("ExchangeAgent" in t1 and "EtfArbAgent" in t2) or (
                "ExchangeAgent" in t2 and "EtfArbAgent" in t1
            ):
                # latency[i,j] = 20000
                latency[i, j] = 5
                ...
                # latency[i,j] = 20000
                latency[i, j] = 1
            elif ("ExchangeAgent" in t1 and "ImpactAgent" in t2) or (
                "ExchangeAgent" in t2 and "ImpactAgent" in t1
            ):
                # latency[i,j] = 20000
                latency[i, j] = 1
