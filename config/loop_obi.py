...
from agent.order_book_balance_agent import OrderBookImbalanceAgent
...
parser.add_argument(
    "-f", "--obi_freq", type=int, default=1e9 * 60, help="OBI subscription frequency"
)
parser.add_argument(
    "-r", "--flat_range", type=float, default=0.1, help="OBI dead zone for staying flat"
)
parser.add_argument(
    "-y",
    "--entry_threshold",
...
parser.add_argument(
    "-e", "--levels", type=int, default=10, help="OBI order book levels to consider"
)
parser.add_argument(
    "-n",
    "--num_simulations",
...
agent_seeds = [
    np.random.RandomState(seed=np.random.randint(low=0, high=2**32))
] * num_agents
...
latency = np.random.uniform(
    low=21000, high=13000000, size=(len(agent_types), len(agent_types))
)
