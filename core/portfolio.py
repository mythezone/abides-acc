...
class Portfolio:
    def __init__(self, initial_cash: float = 0.0):
        self.holdings: Dict[str, int] = {}  # stock -> shares
        ...
        self.last_price: Dict[str, float] = {}  # stock -> last known price for MTM
    ...
    def current_total_value(self, current_prices: Optional[Dict[str, float]] = None) -> float:
        ...
        if current_prices is None:
            ...
            for stock, shares in self.holdings.items():
                total += shares * float(self.last_price.get(stock, 0.0))
        else:
            for stock, shares in self.holdings.items():
                total += shares * float(current_prices.get(stock, 0.0))
        return float(total)

    def apply_trade(self, stock: str, side: str, price: float, quantity: int):
        # Update holdings and cash at execution time
        price = float(round(price, 2))
        qty = int(quantity)
        if side == "buy":
            self.cash -= price * qty
            self.holdings[stock] = int(self.holdings.get(stock, 0) + qty)
        elif side == "sell":
            self.cash += price * qty
            self.holdings[stock] = int(self.holdings.get(stock, 0) - qty)
    ...
    def snapshot(self, date, current_prices: Dict):
        ...
        for stock, shares in self.holdings.items():
            price = current_prices.get(stock, 0.0)
            snapshot_text += f"  {stock}: {shares} shares @ {price:.2f}\n"
        snapshot_text += (
